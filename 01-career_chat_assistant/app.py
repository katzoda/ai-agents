...
def handle_tool_call(tool_calls):
    ...
    for tool_call in tool_calls:
        # LLM is providing back in the response this information - tool name and arguments 
        tool_name = tool_call.function.name

        # arguments (parameters that should be put into that function) and we can strip them out like this:
        # arguments are email, name, notes (described in JSON record_user_details_json)

        # in case of calling record_unknown_question() arguments contain question, name is the name of the tool
        # ChatCompletionMessage(
                            #tool_calls=[ChatCompletionMessageToolCall(id='call_NmZjiCIsvR9tpQ55JuBSXfzG', 
                            #function=Function(arguments='{"question":"Do you hold any certification?"}', name='record_unknown_question'), 
                            #type='function')])
